...
<head>
    ...
    <script type="text/javascript">
        ...
        let server_Token; // 服务器返回的验证令牌
        let publicKeyPem; // 服务器发来的加密公钥
        ...
        function get_clientKey() {// 获取客户端密钥
            ...
            client_privateKeyPem = encrypt.getPrivateKey();
        }
        ...
        function get_PublicKey(key) {// 获取公钥
            publicKeyPem = new JSEncrypt();
            publicKeyPem.setPublicKey(key);
        }


        function encryptString(str) {// 加密密码
            ...
            return publicKeyPem.encrypt(hash); // 返回 Base64 编码的加密数据
        }

        function encryptString_login(str, AES_str) {// 登陆时用的加密密码，还会添加AES密钥
            // 想了想，服务端打算用个固定盐值，这里本来接收由服务端发来的盐值，后来弃用这一策略。
            let saltedString = str + "salt_record";
            // 使用CryptoJS进行SHA-256哈希，发到服务端后加盐
            // 添加使用一次性服务器令牌来防止重放攻击
    ...
    <script type="text/javascript">
        ...
        function get_Ajax_Token(send_data) {// 发送ajax请求，向服务器获取令牌以及加密公钥。
            $.ajax({
                ...
                success: function (response) {//请求成功的回调
                    ...
                    server_Token = token;// 保存令牌
                    if (send_data["verify_type"] === "username_Toke") {//不验证判断
                        if (token != null) {//不验证给了令牌
        ...
        function logOnSubmit() {// 登录提交
            ...
            let AES_str = generate32BitRandomString();
            let AES_key = generateKey(AES_str);
            let AES_iv = generateIV(AES_str);
            let date = {
                verify_type: "logOnSubmit",    // 验证类型，传到服务器后根据类型选择处理函数
                username: username,//用户名
                password: encryptString_login(password, AES_str),// 加密密码
                client_publicKey: AES_encrypt(publicKeyBase64, AES_key, AES_iv),
            }
        ...
        function registerSubmit() {
            ...
            let date = {
                ...
                password: encryptString(password1),// 加密密码
            }
                    ...
                    if (send_data["verify_type"] === "logOnSubmit") {// 登录
                        ...
                        switch (state) {
                            case "OK":
                                ...
                                localStorage.setItem("client_KeyPem", JSON.stringify({
                                    client_publicKeyPem: client_publicKeyPem,
                                    client_privateKeyPem: client_privateKeyPem
                                }));
                                window.location.replace("主页.jsp")// 跳转到主页
                                break;
                            case "Token error":
                                alert("令牌错误，可能令牌已过期");
                                break;
